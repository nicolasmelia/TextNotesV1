			...
			<section class="content">
				<g:if test="${keywordCount == 0}">
					<div class="callout callout-info" style="margin-bottom: 10px!important;">
						...
						It looks like you dont have any keywords yet. Get started by adding keywords below! You can use keywords to draw customers to your business by offering coupons or starting contest. You can also build a larger contact list each time a customer sends a text to your keyword!</div>
				</g:if>
									...
									<tbody>
										<g:if test="${keywordCount > 0 || offset > 0}">
											<g:if test="${keywords != 'NONE'}">
												<g:each in="${keywords}">
													...
													<td>
														<g:formatDate format="MM-dd-yyyy" date="${it.dateEff}" />
														<g:if test="${it.endless}">- Endless</g:if>
														<g:else>-
															<g:formatDate format="MM-dd-yyyy" date="${it.dateExp}" /></g:else>
													</td>
										...
										<g:else>
											<tr onclick="document.location = '${createLink(controller: 'Dashboard', action: 'newKeyWord')}';" class="pointer">
												<td><a href="#"><b>Click here to add your first keyword!</b></a>
												</td>
												<td>-</td>
												<td>-</td>
												<td>-</td>
